        ...
        <nav class="-mb-px flex space-x-4 text-sm leading-5 overflow-x-auto scrollbar-hide text-gray-500">
            ...
            <div @click.away="open = false" class="cursor-pointer flex-col flex hover:bg-gray-200 dark:hover:bg-newgray-700 py-3  rounded @if($nav == 'pages' )border-b-2  dark:border-gray-300 border-newgray-900 @endif" x-data="{ open: false }">
                <button @click="open = !open" class="cursor-pointer  flex hover:bg-gray-200 dark:hover:bg-newgray-700  rounded ">
                    <a  class="px-2 hover:text-newgray-900 dark:hover:text-gray-300 cursor-pointer inline-flex   items-center">Tentang</a>
                ...
                <div x-show="open" style="display: none !important;" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute  w-full mt-8 origin-top-right  shadow-lg md:w-60 z-20">
                  <div class="  bg-gray-100 dark:bg-newgray-700   shadow  flex flex-col gap-2 px-4 py-2 w-full">
                        <a href="{{url('/cms/addabout')}}" class=" hover:underline">Tentang LPRA</a>
                        <a href="{{url('/cms/kekuatan')}}" class=" hover:underline ">Kekuatan LPRA</a>
                        <a href="{{url('/cms/objeksubjek')}}" class=" hover:underline ">Objek-Subjek LPRA</a>
                        <a href="{{url('/cms/penguatan')}}" class=" hover:underline ">Penguatan Organisasi LPRA</a>
                        <a href="{{url('/cms/advokasi')}}" class=" hover:underline ">Advokasi LPRA</a>
                        <a href="{{url('/cms/carakerja')}}" class=" hover:underline ">Cara Kerja LPRA</a>
                        <a href="{{url('/cms/database')}}" class=" hover:underline ">Database LPRA</a>
