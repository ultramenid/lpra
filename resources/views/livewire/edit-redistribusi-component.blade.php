<div class="">
    ...
    <div class=" border-b border-gray-300 dark:border-opacity-20 ">
        <div class="max-w-6xl mx-auto px-6  flex justify-between  py-16">
            <h1 class="sm:text-4xl text-xl text-newgray-900 dark:text-newgray-300 font-semibold ">New redistribusi</h1>
            <div class="z-30">
                <button wire:loading.remove wire:target='storeUpdates'  wire:click='storeUpdates' id="btnStore" class="inline-flex sm:px-16 px-8 sm:py-2 py-1 rounded dark:hover:bg-newgray-900 dark:hover:border-gray-200 dark:hover:text-gray-200 hover:bg-white hover:text-newgray-900 border hover:border-newgray-900 bg-newgray-900 dark:bg-gray-100 text-newgray-100 dark:text-newgray-900">
                    Save
                </button>
                <button wire:loading wire:target='storeUpdates' id="btnStore" class="inline-flex sm:px-16 px-8 sm:py-2 py-1 rounded dark:hover:bg-newgray-900 dark:hover:border-gray-200 dark:hover:text-gray-200 hover:bg-white hover:text-newgray-900 border hover:border-newgray-900 bg-newgray-900 dark:bg-gray-100 text-newgray-100 dark:text-newgray-900">
        ...
        <div class="sm:col-span-9 col-span-12 " >
                ...
                <div class="flex items-center justify-center px-2 py-2 border border-dashed border-gray-400 rounded" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false; progress = 5" x-on:livewire-upload-error="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                    <label class="cursor-pointer">
                        @if ($uphoto)
                            ...
                            @else
                                <img src="{{ asset('/storage/photos/shares/'.$uphoto)  }}" alt="" class=" mx-auto h-96 w-full rounded object-contain  ">
                            @endif
                        @endif
            ...
            <div >
                ...
                <div class="w-full border border-gray-300 dark:border-opacity-20  px-6 py-6 mb-6">
                    <h1 class="text-2xl font-semibold  text-newbg-newgray-900 dark:text-gray-300 mb-4">file pdf</h1>
                    <input wire:ignore  type="file" accept="application/pdf" class="bg-gray-100  text-gray-700  rounded w-full border mb-2  py-2 px-4 focus:outline-none border-gray-300 dark:border-opacity-20"  wire:model.defer='pdffile' placeholder="upload. . . ">
                    @if($pdffile)
                        <a class="mt-8 text-green-500 underline" href="{{ asset('storage/files/lampiran/'.$pdffile) }}">File Redistribusi {{$titleID}} </a>
                    @endif
                </div>
            </div>
